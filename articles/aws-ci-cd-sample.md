...
| フェーズ | 対応するサービス                       | 概要                                                                         |
| :--- | :----------------------------- | :------------------------------------------------------------------------- |
| トリガー | Amazon EventBridge, CodeCommit | CodeCommit上の変化がCloudWatch Eventとして発行されるので、その中からフィルタして目的のイベントが置きた場合に処理を開始する |
...
- AWS　マネジメントコンソールのサービス一覧から、S3を選択します。
...
- AWS　マネジメントコンソールのサービス一覧から、CodeBuildを選択します。
...
- ビルドプロジェクトの一覧から作成したビルドプロジェクトを選択して「ビルドを開始」を押すことで、ビルドを実行することができます。エラーが置きた場合は、ログを読んで修正しましょう。
